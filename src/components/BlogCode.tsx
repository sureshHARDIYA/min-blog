...
const KEYWORDS: Record<string, ReadonlySet<string>> = {
  javascript: new Set([
    ...
    'return', 'static', 'switch', 'throw', 'true', 'try', 'typeof', 'undefined',
    'while', 'yield',
  ]),
  python: new Set([
...
export function BlogCode({
  ...
  const language = LANGUAGE_ALIASES[languageName ?? ''] ?? languageName ?? 'text';
...
export function BlogList({ children }: ComponentPropsWithoutRef<'ul'>) {
  ...
  const isTaskList = items.some(
    (item) => isValidElement<{ children?: ReactNode }>(item) && taskDetails(item.props.children),
  );
